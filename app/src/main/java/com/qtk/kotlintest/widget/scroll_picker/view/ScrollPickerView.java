package com.qtk.kotlintest.widget.scroll_picker.view;
...
import com.qtk.kotlintest.widget.scroll_picker.util.ScreenUtil;
...
public class ScrollPickerView extends RecyclerView {
    ...
    public void onDraw(Canvas c) {
        ...
        if (!mFirstAmend) {
            mFirstAmend = true;
            ((LinearLayoutManager) getLayoutManager()).scrollToPositionWithOffset(getItemSelectedPosition(), 0);
        }
    }
    ...
    private int getItemSelectedPosition() {
        IPickerViewOperation operation = (IPickerViewOperation) getAdapter();
        if (operation != null) {
            return operation.getSelectedItemPosition();
        }
        return 0;
    }
    ...
    private void freshItemView() {
        for (int i = 0; i < getChildCount(); i++) {
            float itemViewY = getChildAt(i).getTop() + mItemHeight / 2f;
